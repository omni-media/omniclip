...
const tools = [
    // ── Query Tools ──────────────────────────────────────────────────
    {
        ...
        description: "Get the complete timeline state including all effects, tracks, filters, animations, transitions, and current playback position. Call this before making changes to understand the current project state.",
        inputSchema: {
            type: "object",
            properties: {},
            required: [],
    ...
    },
    {
        ...
        description: "Get project metadata: name, ID, resolution, aspect ratio, standard, bitrate, and timeline statistics (number of effects, tracks, total duration).",
        inputSchema: {
            type: "object",
            properties: {},
            required: [],
    ...
    {
        name: "get_effects",
        description: "Get all effects on the timeline, optionally filtered by kind (video/audio/image/text) or track number.",
        inputSchema: {
            type: "object",
            properties: {
                kind: {
    ...
    },
    {
        ...
        description: "List all available filter types (e.g., BlurFilter, GrayscaleFilter, AdjustmentFilter) that can be applied to video and image effects.",
        inputSchema: {
            type: "object",
            properties: {},
            required: [],
    ...
    },
    {
        ...
        description: "List all available animation types for effects (e.g., fade-in, slide-out, spin-in, zoom-out, etc.).",
        inputSchema: {
            type: "object",
            properties: {},
            required: [],
    ...
    },
    {
        ...
        description: "List all available GL transition types that can be applied between two adjacent video/image effects on the same track.",
        inputSchema: {
            type: "object",
            properties: {},
            required: [],
    ...
    // ── Effect CRUD ──────────────────────────────────────────────────
    {
        ...
        description: "Add a new text effect to the timeline. Text appears as an overlay on the video canvas. If start_at_position and track are not specified, the effect is placed automatically at the first available position.",
        inputSchema: {
            type: "object",
            properties: {
    ...
    {
        name: "remove_effect",
        description: "Remove an effect from the timeline by its ID. Also removes associated animations, transitions, and filters.",
        inputSchema: {
            type: "object",
            properties: {
    ...
    // ── Effect Positioning ───────────────────────────────────────────
    {
        ...
        description: "Set the timing properties of an effect: its position on the timeline, duration, start/end trim points, and track assignment. Only provided properties are updated.",
        inputSchema: {
            type: "object",
            properties: {
    ...
    // ── Text Styling ─────────────────────────────────────────────────
    {
        ...
        description: "Set one or more style properties on a text effect. All properties are optional — only provided ones are updated.",
        inputSchema: {
            type: "object",
            properties: {
    ...
    {
        name: "add_transition",
        description: "Add a GL transition between two adjacent video/image effects on the same track. The outgoing effect must end exactly where the incoming effect begins (they must be touching).",
        inputSchema: {
            type: "object",
            properties: {
    ...
    {
        name: "add_animation",
        description: "Add an animation to a video or image effect. Animations are entrance ('in') or exit ('out') effects like fade, slide, spin, bounce, wipe, blur, zoom.",
        inputSchema: {
            type: "object",
            properties: {
    ...
    {
        name: "add_filter",
        description: "Add a visual filter to a video or image effect. Use list_available_filters to see available types. Filters can only be applied to video and image effects (not audio or text).",
        inputSchema: {
            type: "object",
            properties: {
                effect_id: { type: "string", description: "ID of the video or image effect" },
...
async function handleToolCall(name, args) {
    switch (name) {
        ...
        case "add_text_effect":
            return JSON.stringify(await bridge.sendAction("add_text_effect", {
                text: args.text,
        ...
        case "remove_effect":
            return JSON.stringify(await bridge.sendAction("remove_effect", { effect_id: args.effect_id }), null, 2);
        case "remove_all_effects":
            return JSON.stringify(await bridge.sendAction("remove_all_effects", {}), null, 2);
        // ── Effect Positioning ──
        case "set_effect_timing":
            return JSON.stringify(await bridge.sendAction("set_effect_timing", {
        ...
        case "set_effect_position_on_canvas":
            return JSON.stringify(await bridge.sendAction("set_effect_position_on_canvas", {
                effect_id: args.effect_id,
                x: args.x,
                y: args.y,
            }), null, 2);
        case "rotate_effect":
            return JSON.stringify(await bridge.sendAction("rotate_effect", {
                effect_id: args.effect_id,
                rotation: args.rotation,
            }), null, 2);
        case "scale_effect":
            return JSON.stringify(await bridge.sendAction("scale_effect", {
                effect_id: args.effect_id,
                scaleX: args.scaleX,
                scaleY: args.scaleY,
            }), null, 2);
        case "resize_effect":
            return JSON.stringify(await bridge.sendAction("resize_effect", {
                effect_id: args.effect_id,
                width: args.width,
                height: args.height,
            }), null, 2);
        ...
        case "set_text_properties": {
            ...
            return JSON.stringify(await bridge.sendAction("set_text_properties", { effect_id, ...properties }), null, 2);
        }
        // ── Track Management ──
        case "add_track":
            return JSON.stringify(await bridge.sendAction("add_track", {}), null, 2);
        case "remove_track":
            return JSON.stringify(await bridge.sendAction("remove_track", { track_id: args.track_id }), null, 2);
        case "toggle_track_muted":
            return JSON.stringify(await bridge.sendAction("toggle_track_muted", { track_id: args.track_id }), null, 2);
        case "toggle_track_visibility":
            return JSON.stringify(await bridge.sendAction("toggle_track_visibility", { track_id: args.track_id }), null, 2);
        case "toggle_track_locked":
            return JSON.stringify(await bridge.sendAction("toggle_track_locked", { track_id: args.track_id }), null, 2);
        // ── Transitions ──
        case "add_transition":
            return JSON.stringify(await bridge.sendAction("add_transition", {
        ...
        case "remove_transition":
            return JSON.stringify(await bridge.sendAction("remove_transition", { transition_id: args.transition_id }), null, 2);
        case "set_transition_duration":
            return JSON.stringify(await bridge.sendAction("set_transition_duration", {
                transition_id: args.transition_id,
                duration: args.duration,
            }), null, 2);
        // ── Animations ──
        case "add_animation":
            return JSON.stringify(await bridge.sendAction("add_animation", {
                effect_id: args.effect_id,
                animation_name: args.animation_name,
                duration: args.duration ?? 500,
            }), null, 2);
        case "remove_animation":
            return JSON.stringify(await bridge.sendAction("remove_animation", {
                effect_id: args.effect_id,
                type: args.type,
            }), null, 2);
        case "set_animation_duration":
            return JSON.stringify(await bridge.sendAction("set_animation_duration", {
                effect_id: args.effect_id,
                duration: args.duration,
            }), null, 2);
        // ── Filters ──
        case "add_filter":
            return JSON.stringify(await bridge.sendAction("add_filter", {
                effect_id: args.effect_id,
                filter_type: args.filter_type,
            }), null, 2);
        case "remove_filter":
            return JSON.stringify(await bridge.sendAction("remove_filter", {
                effect_id: args.effect_id,
                filter_type: args.filter_type,
            }), null, 2);
        // ── Playback ──
        case "set_playhead":
            return JSON.stringify(await bridge.sendAction("set_playhead", { timecode: args.timecode }), null, 2);
        case "play":
        ...
        case "set_project_name":
            return JSON.stringify(await bridge.sendAction("set_project_name", { name: args.name }), null, 2);
        case "set_project_resolution":
            return JSON.stringify(await bridge.sendAction("set_project_resolution", {
                width: args.width,
                height: args.height,
            }), null, 2);
        case "set_project_standard":
            return JSON.stringify(await bridge.sendAction("set_project_standard", { standard: args.standard }), null, 2);
        case "set_project_aspect_ratio":
            return JSON.stringify(await bridge.sendAction("set_project_aspect_ratio", { aspect_ratio: args.aspect_ratio }), null, 2);
        case "undo":
